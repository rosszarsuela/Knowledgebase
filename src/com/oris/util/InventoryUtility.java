...
public class InventoryUtility {
    ...
    /**
     * Retrieves the Fma object of the login user
     */
    public static Users getLoginUser() {
    	Users user = new Users();
    	user.setUsername(getLoginUsername());
	...
	public static synchronized String generateSequence(Set<Integer> serialNos) {
		Integer nextNo = null;
		Integer prevNo = null;
		...
		StringBuffer sb = new StringBuffer();
		String endStr = "";
		for (Integer i : serialNos) {
			Integer currNo = i;
			
			if(prevNo == null) {
				isNewSeq = true;
			} else if(prevNo != null && currNo.equals(nextNo)) {
				isNewSeq = false;
				count++;
			} else if(prevNo != null && !currNo.equals(nextNo)) {
				isNewSeq = true;
			...
			if(isNewSeq) {
				if(sequence.size() > 0) {
					endStr = sequence.get(sequence.size() - 1).toString();
					sequence.clear();
					
					sb.append("-");
//					sb.append(StringUtils.leftPad(endStr.toString(), Integer.parseInt(Config.getProperties("mis.serial.no.length").toString()), "0"));
					sb.append(endStr.toString());
					sb.append(",");
				}
				
//				sb.append(StringUtils.leftPad(currNo.toString(), Integer.parseInt(Config.getProperties("mis.serial.no.length").toString()), "0"));
				sb.append(currNo.toString());
				sb.append(",");
			} else {
				sequence.add(currNo.toString());
			}			
		...
		String converted = "";
		if(!isNewSeq && sequence.size() > 0) {
			endStr = sequence.get(sequence.size() - 1).toString();
			sb.append("-");
//			sb.append(StringUtils.leftPad(endStr.toString(), Integer.parseInt(Config.getProperties("mis.serial.no.length").toString()), "0"));
			sb.append(endStr.toString());
			converted = sb.toString().replaceAll(",-", "-");
		} else {
			String seq = sb.toString().replaceAll(",-", "-");
			int index = seq.toString().lastIndexOf(",");
			converted = seq.toString().substring(0, index);			
		}
