...
public class AdjustmentBillingUtil extends BasePDFGenUtil{
	private static final String directory = "D:/EGG Projects/HMODemo/Per Module Docs/Admin/Billing/PDF/AdjustmentBilling.pdf";
	...
	private static PdfPTable createCoverPageTable() {
		PdfPTable coverPageTable = new PdfPTable(4);
		coverPageTable.setWidthPercentage(100);
		
		int colspan = coverPageTable.getNumberOfColumns();
				
		coverPageTable.addCell(PDFUtil.createPdfPCell(dateFormat.format(new Date()),  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("MR. JONATHAN RUBRICO",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("TRINITY HEALTHCARE SERVICES, INC.",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("5/f Philcox Building, 172 Salcedo St. Legaspi Village Makati City",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("RE: NSM GROUP OF PRINCIPALS",  helvetica10Bold, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("       "+"NSM GROUP OF PRINCIPALS (WISBY TANKERS - RATINGS)",  helvetica10Bold, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("       "+"HCP08-0904",  helvetica10Bold, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(padding, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("Dear Madam/Sir:",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("\t\tPlease find attached statement with no. 12-050682-0077 as Adjustment for January in the amount of Php 49,625.64",  helvetica10, ALIGN_LEFT, padding, colspan, false, WHITE));		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		
		coverPageTable.addCell(PDFUtil.createPdfPCell("\t\tWe'd appreciate your full settlement within 15 calendar days for us to be able to clear your accountabilities. Otherwise, we regret to advice that we shall be constrained to suspend availments and hold reimbursements.",  helvetica10, ALIGN_LEFT, padding, colspan, false, WHITE));		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
				
		coverPageTable.addCell(PDFUtil.createPdfPCell("\t\tPlease make all remittances payable to COCOLIFE. Should you have any query, you may call our direct line at  813-3000 loc. 105.",  helvetica10, ALIGN_LEFT, padding, colspan, false, WHITE));		
		coverPageTable.addCell(PDFUtil.createBlankPdfPCell(5, colspan, WHITE));
		...
		coverPageTable.addCell(PDFUtil.createPdfPCell("Thank you.",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));		
		...
		coverPageTable.addCell(PDFUtil.createPdfPCell("_____________________________",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createPdfPCell("Ronald Valencia",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
		coverPageTable.addCell(PDFUtil.createPdfPCell("HealthCare Division",  helvetica10, ALIGN_LEFT, padding, colspan, true, WHITE));
	...
	private static PdfPTable createBillingNotice2Table() throws DocumentException {
...
//		billingNotice2Table.setWidths(new float[]{1f, 1f ,1f , 0.5f});
		...
		billingNotice2Table.addCell(PDFUtil.createPdfPCell("Adjustment Billing for September",  helvetica10, ALIGN_LEFT, padding,  true, WHITE));
